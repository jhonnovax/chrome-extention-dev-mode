...
- **Status (2026-09-25):** **Phase 0 shipped** as extension 1.4.0 on the current chrome.debugger engine: config in `chrome.storage.sync` (`sync-schema.js`, per-rule keys), manifest `key` (ID `kiodgandllfdichppoigkhkdjbajpnpn`), and Map Local served straight from disk with `fetch('file://…')` (needs "Allow access to file URLs"), so the old static server and its launchd agent were **removed**. The 1.x `/__config { mounts }` compatibility in this doc is therefore moot: `local-server/` would be recreated from scratch for the proxy engine and the extension would keep serving Map Local from disk in Chrome (the proxy only needs the files for Safari/Firefox). Everything from phase 1 on is still design only.
